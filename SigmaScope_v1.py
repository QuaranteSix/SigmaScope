# ============================================================
# SIGMASCOPE — Architecture des fichiers et dossiers
# ============================================================
#
#  SigmaScope_18.py                ← Script principal de l'application
#
#  SigmaScope_Assets/              ← Données de référence des indices boursiers
#  ├── Indice_Ticker_Wikipedia.csv     Composants des indices (S&P 500, CAC 40…)
#  │                                   téléchargés depuis Wikipedia via ⚙️ Configuration.
#  │                                   Colonnes : index_key, Ticker, Company
#  ├── Indice_Ticker_Customization.csv Indices/ETF personnalisés ajoutés manuellement.
#  │                                   Même format que Indice_Ticker_Wikipedia.csv.
#  └── Indice_List_Customization.csv   Liste de tickers libres (grands indices,
#                                      ex : ^GSPC, ^NDX…).
#                                      Colonnes : Ticker, Company
#
#  SigmaScope_Watchlist/           ← Watchlists de l'utilisateur (multi-watchlist)
#  ├── watchlists_index.csv            Index des watchlists existantes.
#  │                                   Colonne : name (ex : "Ma Watchlist", "CTO"…)
#  └── <Nom_Watchlist>.csv             Un fichier CSV par watchlist.
#                                      Colonnes : ticker, company, ajout_date,
#                                                 note, prix_achat
#
#  SigmaScope_Exports/             ← Dossier réservé aux exports futurs (vide au départ)
#
# ============================================================
...
def _wl_path(name):
    ...
    return os.path.join(WATCHLIST_DIR, f"{safe}.csv")

def load_wl_index():
    """Retourne la liste des watchlists disponibles (noms)."""
    _ensure_wl_dir()
    # Migration automatique de l'ancien watchlist.csv vers "Ma Watchlist"
    if os.path.exists(WATCHLIST_CSV) and not os.path.exists(WATCHLIST_INDEX):
        try:
            df_old = pd.read_csv(WATCHLIST_CSV, dtype=str)
            df_old.to_csv(_wl_path("Ma Watchlist"), index=False)
        except:
            pass
        pd.DataFrame({"name": ["Ma Watchlist"]}).to_csv(WATCHLIST_INDEX, index=False)
    if os.path.exists(WATCHLIST_INDEX):
        try:
            df_idx = pd.read_csv(WATCHLIST_INDEX, dtype=str)
            names = df_idx["name"].dropna().tolist()
            if names:
                return names
        except:
            pass
...
def save_wl_index(names):
    _ensure_wl_dir()
    pd.DataFrame({"name": names}).to_csv(WATCHLIST_INDEX, index=False)

def load_watchlist(name=None):
    if name is None:
        name = st.session_state.get("active_watchlist", load_wl_index()[0])
    path = _wl_path(name)
...
def save_watchlist(df, name=None):
    if name is None:
        name = st.session_state.get("active_watchlist", load_wl_index()[0])
    _ensure_wl_dir()
    df[WATCHLIST_COLS].to_csv(_wl_path(name), index=False)

def add_to_watchlist(ticker, company="", name=None):
    df = load_watchlist(name)
    ...
    save_watchlist(df, name)
    return True

def remove_from_watchlist(ticker, name=None):
    df = load_watchlist(name)
    df = df[df["ticker"].str.upper() != ticker.strip().upper()]
    save_watchlist(df, name)

def is_in_watchlist(ticker, name=None):
    df = load_watchlist(name)
    return ticker.strip().upper() in df["ticker"].str.upper().values

def get_pru(ticker):
    """
    Cherche le PRU (prix_achat) d'un ticker dans toutes les watchlists.
    Retourne un float si trouvé et valide, sinon None.
    """
    tkr_up = ticker.strip().upper()
    for wl_name in load_wl_index():
        df = load_watchlist(wl_name)
...
def create_watchlist(name):
    ...
    if name not in names:
        ...
        save_wl_index(names)
        # Créer fichier vide
        pd.DataFrame(columns=WATCHLIST_COLS).to_csv(_wl_path(name), index=False)
    return names
...
def load_all_indices():
    ...
    if os.path.exists(COMPONENTS_CSV):
        df = pd.read_csv(COMPONENTS_CSV, dtype=str)
        for key in df["index_key"].unique():
            result[key] = df[df["index_key"] == key][["Ticker", "Company"]].reset_index(drop=True)
    if os.path.exists(CUSTO_CSV):
        df_custo = pd.read_csv(CUSTO_CSV, dtype=str)
...
def load_indices_list():
    if not os.path.exists(INDICES_LIST_CSV):
        return None
    df = pd.read_csv(INDICES_LIST_CSV, dtype=str)
    if "Ticker" in df.columns and "Company" in df.columns:
        return df[["Ticker", "Company"]].dropna().reset_index(drop=True)
    return None
...
def save_index_to_master_csv(index_key, df_new):
    df_new = df_new.copy()
    df_new["index_key"] = index_key
    if os.path.exists(COMPONENTS_CSV):
        df_existing = pd.read_csv(COMPONENTS_CSV, dtype=str)
        df_existing = df_existing[df_existing["index_key"] != index_key]
        df_final = pd.concat([df_existing, df_new[["index_key", "Ticker", "Company"]]], ignore_index=True)
    else:
        df_final = df_new[["index_key", "Ticker", "Company"]]
    df_final.to_csv(COMPONENTS_CSV, index=False)


# ============================================================
# CACHE YFINANCE — TTL 1h
# ============================================================
...
# Contraintes période max par intervalle intraday
...
@st.cache_data(ttl=300, show_spinner=False)   # TTL plus court pour l'intraday
def get_history_intraday(ticker, period, interval):
    """Récupère l'historique avec un intervalle personnalisé (intraday ou daily)."""
...
def get_recommendations(ticker):
    """Retourne l'historique des recommandations analysts ou None."""
    try:
        t = yf.Ticker(ticker)
...
def get_calendar(ticker):
    """Retourne le calendrier (dividende, earnings) ou None."""
    try:
        t = yf.Ticker(ticker)
        cal = t.calendar
        return cal
...
def get_live_quote(ticker):
    """Retourne dict {name, price, change_pct, currency} ou None."""
    try:
        t = yf.Ticker(ticker)
        info = t.info
        ...
        if price and prev and prev != 0:
            ...
            change_pct = None
        return {"name": name, "price": price, "change_pct": change_pct, "currency": curr}
    except:
        return None
@st.cache_data(ttl=60, show_spinner=False)
def get_ticker_currency(ticker):
    """Retourne la devise du ticker Yahoo Finance (ex: 'USD', 'EUR', 'CAD')."""
    try:
        info = yf.Ticker(ticker).info
        return info.get("currency", "EUR") or "EUR"
    except:
        return "EUR"

@st.cache_data(ttl=3600, show_spinner=False)
def get_eur_to_currency_rate(target_currency):
    """
    Retourne le taux de change EUR → target_currency.
    Utilise yfinance avec le symbole EUR<CURRENCY>=X.
    Retourne 1.0 si la devise cible est EUR ou si la conversion échoue.
    """
...
with st.sidebar:
    ...
    if all_data:
        ...
        for key, df_idx in all_data.items():
            ...
            label = get_label(key)
            if os.path.exists(CUSTO_CSV):
                df_custo_keys = pd.read_csv(CUSTO_CSV, dtype=str)["index_key"].unique()
                prefix = "🔧 " if key in df_custo_keys else ""
            else:
                prefix = ""
            st.markdown(f"• {prefix}{label}: **{n}** valeurs")
        if os.path.exists(CUSTO_CSV):
            st.caption("🔧 = indices/ETF depuis le fichier custom")
    else:
...
# ============================================================
    ...
    if all_data:
        rows_main = []
        if os.path.exists(COMPONENTS_CSV):
            df_master = pd.read_csv(COMPONENTS_CSV, dtype=str)
    ...
    custo_status = f"✅ présent ({len(pd.read_csv(CUSTO_CSV))} lignes)" if os.path.exists(CUSTO_CSV) else "⚠️ absent"
    wl_count = sum(len(load_watchlist(n)) for n in load_wl_index())
    wl_status    = f"✅ {len(load_wl_index())} liste(s), {wl_count} entrée(s) au total"
    st.code(
        f"Dossier assets      : {ASSETS_DIR}\n"
        f"  Composants        : {COMPONENTS_CSV}\n"
        f"  Liste indices     : {INDICES_LIST_CSV}\n"
        f"  Custom            : {CUSTO_CSV}  →  {custo_status}\n"
        f"Dossier watchlists  : {WATCHLIST_DIR}  →  {wl_status}\n"
        f"Dossier exports     : {EXPORTS_DIR}",
        language=""
    )